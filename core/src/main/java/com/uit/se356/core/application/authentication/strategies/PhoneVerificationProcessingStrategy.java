...
public class PhoneVerificationProcessingStrategy implements ProcessVerificationStrategy {
  ...
  public VerificationResult process(CodePurpose purpose, String recipient, String code) {
    ...
    cacheRepository.set(verifiedCacheKey.toString(), recipient, Duration.ofMinutes(10)); // 10 phút
    // Trả về token đã tạo
    return new VerificationResult(token, Duration.ofMinutes(10).toSeconds());
  }
}
