...
public class PhoneVerificationSendingStrategy implements SendVerificationStrategy {
  ...
  public void send(
    ...
    long expirationSeconds = verificationConfigPort.getSmsOtpExpiration();
    cacheRepository.set(cacheKey.toString(), otpCode, Duration.ofSeconds(expirationSeconds));
    ...
    senders.stream()
        ...
        .forEach(
            sender -> {
              sender.send(recipient, otpCode, purpose);
            });
  }
}
